...
IF EXIST %boost_dir% IF %UPDATE_SOURCE%==NO GOTO BUILD_BOOST
...
git submodule update --init libs/date_time libs/type_index libs/any libs/regex libs/function libs/algorithm libs/lexical_cast libs/concept_check libs/numeric libs/integer libs/array libs/container libs/move libs/math libs/tokenizer libs/uuid

REM next modules are required for boost::beast (REST server/client library)
git submodule update --init libs/beast libs/intrusive libs/optional
...
:BUILD_BOOST
IF EXIST %boost_dir%\stage\x64\lib\*.lib IF %REBUILD%==NO GOTO END_BOOST
pushd %boost_dir%
...
rem Build DLLs
 b2 -j%cores% toolset=%msvcver% variant=release,debug address-model=64 architecture=x86 link=shared threading=multi runtime-link=shared %MODULES% stage --stagedir=stage/x64
popd
:END_BOOST
